...
describe('Configure Workflow - Workflow Management', () => {
  beforeAll(async () => {
  ...
  it('should be able to display Issue Category Details', async () => {
    ...
    issueNumber = await user.createNewIssue(issueCreateData);
  ...
  it('should verify issue created appears in Issue Management Page', async () => {
    ...
    const issueDetails = await issueManagemwentPage.getIssueDetailsById(issueTofetch);
  ...
  it('created categories SHOULD ONLY be available to selected asset type', async () => {
    ...
    console.log(`category Options: `, await issueSnapshotPage.getCategoryOptions().getText());
