...
const uniqueSmal7 = (Date.now()).toString().slice(5)
...
describe('asset360 risk matrix tab and list tab, then choose Save and Continue, create list', function () {
  ...
  it('should drag and select a risk matrix blocks, zoom-in ', async () => {
    ...
    var EC = protractor.ExpectedConditions;
    browser.sleep(3500);
    await browser.wait(async () => await page.matrixHideSpnnr.isPresent());
  ...
  it('should click on list tab and click on save and continue', async () => {
    ...
    await browser.wait(EC.presenceOf(page.saveContiBtn), 128000);
    browser.sleep(3300);
    page.saveContiBtn.click();
    page.saveQsrchTitleTbox.clear();
    await page.saveQsrchTitleTbox.sendKeys(uniqueSmal7);
    await browser.wait(EC.presenceOf(page.saveQsOKBtn), 128000);
    browser.sleep(3300);
  ...
  it('should go to risk matrix tab then, clear list, clear zoom-in filter', async () => {
    console.log('Step 7');
    var EC = protractor.ExpectedConditions;
    browser.sleep(3500);
    await browser.actions().mouseMove(page.RMtab).click().perform();
    browser.wait(EC.visibilityOf(page.raMatrixFilter), 28000);
    await browser.wait(EC.elementToBeClickable(page.raMatrixFilter), 128000);
    browser.sleep(3500);
    await browser.actions().mouseMove(page.raMatrixFilter).perform();
...
describe('asset360 risk matrix tab and list tab, then choose Save and Continue', function () {
  ...
  it('should drag and select a risk matrix blocks, zoom-in ', async () => {
    console.log('Step 8');
    var EC = protractor.ExpectedConditions;
    const activeSearch = element(by.cssContainingText('[ng-repeat="item in tagSearchPopupVM.quickSearches"]', uniqueSmal7));
    browser.sleep(3500);
    await browser.actions().mouseMove(page.raMtrxSrchDd).click().perform();
    helper.waitForVisible(activeSearch, 8000)
    await browser.actions().mouseMove(activeSearch).click().perform();
    browser.sleep(3500);
    await browser.wait(async () => await page.matrixHideSpnnr.isPresent());
    browser.actions().
      mouseDown(element(by.css('#grid-chart > svg > g:nth-child(1) > rect:nth-child(91)'))).
  ...
  it('should ra map tab then click on the search filter button', async () => {
    ...
    await page.searchTboxMapFltr.sendKeys('asset=ug_cable*');
    await browser.actions().mouseMove(page.raSerchFltrBtn).click().perform();
    browser.sleep(3000);
    await browser.actions().mouseMove(page.mapFilterBtn).click().perform();
    await browser.wait(EC.invisibilityOf(element(by.className('mapLoadingSpinner'))), 128000);
  ...
  it('should click on list tab and click on save and continue', async () => {
    ...
    page.saveContiBtn.click();
    await browser.wait(EC.invisibilityOf(page.listUpdSaved), 128000);
  ...
  it('should check list tab details is correct', async () => {
    ...
    browser.wait(EC.visibilityOf(page.srchListTbox), 28000);
    console.log('text: ', await page.srchListTbox.getAttribute('value'));
    expect(await page.srchListTbox.getAttribute('value')).toEqual('asset=ug_cable*');
    await browser.actions().mouseMove(page.toggleListSrch).click().perform();
    expect(page.RMzoomInApld.isPresent()).toBe(true, 'Zoom-in text applied is seen');
    browser.waitForAngular();
  });
  it('should clear list, clear zoom-in filter', async () => {
    console.log('Step 12');
    var EC = protractor.ExpectedConditions;
    browser.sleep(3500);
    await browser.actions().mouseMove(page.toggleListSrch).click().perform();
...
describe('asset360 risk matrix tab and list tab, then choose Retain Search but Dont Save Yet', function () {
  ...
  it('should drag and select a risk matrix blocks, zoom-in ', async () => {
    ...
    await browser.actions().mouseMove(page.RMtab).click().perform();
    browser.sleep(3500);
    await browser.actions().mouseMove(page.ClearSrchContinue).click().perform();
    browser.sleep(3500);
    await browser.wait(async () => await page.matrixHideSpnnr.isPresent());
    browser.actions().
