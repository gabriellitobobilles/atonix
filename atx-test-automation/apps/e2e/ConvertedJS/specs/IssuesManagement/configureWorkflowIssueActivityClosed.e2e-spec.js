...
describe('Issues Management - Configure Workflow - Issue Activity Closed', function () {
    ...
    it('User should be able to add all action details. Name, Recipients, Subject, Message Body', function () { return tslib_1.__awaiter(_this, void 0, void 0, function () {
        ...
        return tslib_1.__generator(this, function (_a) {
            ...
            expect(actionsFromSiteEmail.$$('.select-settings').get(0).getText()).toMatch(openEmailData.recipient);
            expect(actionsFromSiteEmail.$$('.select-settings').get(1).getText()).toMatch(openEmailData.subject);
            expect(actionsFromSiteEmail.$$('.select-settings').get(2).getText()).toMatch(openEmailData.messageBody);
